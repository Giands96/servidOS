...
public class LoginAttemptService {
    ...
    private final Map<LoginAttemptKey, Estado> estados = new ConcurrentHashMap<>();

    /** Lanza BusinessException genérica si la clave está bloqueada. */
    public void exigirPermitido(String email, String ip) {
        Estado estado = estados.get(new LoginAttemptKey(email, ip));
    ...
    public void registrarFallo(String email, String ip) {
        ...
        estados.compute(new LoginAttemptKey(email, ip), (clave, previo) -> {
    ...
    public void limpiar(String email, String ip) {
        estados.remove(new LoginAttemptKey(email, ip));
    }
}
